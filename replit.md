...
**December 16, 2025** - PIN Lock Persistence Fix:
- Fixed PIN lockout bypass by implementing localStorage persistence
- Failed attempts and lock timestamps now survive page refreshes
- Countdown timer shows remaining lockout seconds
- Lock state properly cleared on successful PIN verification
- Prevents users from bypassing 30-second lockout by refreshing the page

**December 16, 2025** - Device Reconnect Button Enhancement:
- Enhanced refresh button on device cards to retry MQTT broker connection
- Added `reconnectDevice` method in mqtt-client.ts with Promise-based connection verification
- Created POST /api/devices/:deviceId/reconnect API endpoint
- Frontend uses proper useMutation/apiRequest flow with cache invalidation
- Connection verification waits for MQTT 'connect' event and subscription success
- 15-second timeout for connection attempts
- Proper error handling updates device status to offline if connection fails

**December 13, 2025** - Professional Excel & PDF Export Enhancements:
- Upgraded Excel export to use ExcelJS library for full styling support
- Added professional header section with title, device info, and generation date
- Implemented color-coded alert status cells (Green=Normal, Yellow=Moderate, Red=Completely Drunk)
- Added alternating row colors for improved readability
- Blue header row with white bold text
- Proper column widths and cell borders throughout
- Added GMR company logo to PDF export header
- Both worksheets (Device Data and Offline Events) now have professional styling
