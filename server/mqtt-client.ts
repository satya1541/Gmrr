...
class MqttClientService {
  ...
  private async connectToDeviceBrokers() {
    try {
      ...
      // Failed to connect to device brokers
    }
  }
  ...
  private async connectToDevice(device: any) {
    try {
      ...
      const options = {
        ...
        protocolVersion: (protocol === 'WS' || protocol === 'WSS') ? 4 : 3, // Use v4 for WebSocket, v3 for regular MQTT
        keepalive: 60,
      ...
      client.on('connect', () => {
        ...
        client.subscribe(device.mqttTopic, { qos: 1 }, (error) => {
          if (error) {
    ...
    try {
      ...
      client.on('error', (error) => {
        ...
        if (error.message.includes('Invalid header flag bits') || error.message.includes('protocol')) {
          ...
          setTimeout(() => this.retryWithDifferentProtocol(device, protocolVersion + 1), 2000);
        } else {
    ...
    try {
      ...
      for (const device of devices) {
        if (device.mqttTopic && !this.subscribedTopics.has(device.mqttTopic)) {
          this.client.subscribe(device.mqttTopic, { qos: 1 }, (error) => {
            if (error) {
              // Failed to subscribe to topic
            } else {
              // Subscribed to topic
  ...
  private async handleMessage(topic: string, message: Buffer, device: any) {
    try {
      ...
      try {
        ...
        // Extract alcohol_level from different possible field names
        let alcoholLevel = 0;
  ...
  async subscribeToNewDevice(mqttTopic: string) {
    if (this.client && mqttTopic && !this.subscribedTopics.has(mqttTopic)) {
      this.client.subscribe(mqttTopic, { qos: 1 }, (error) => {
        if (error) {
