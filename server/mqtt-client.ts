...
class MqttClientService {
  ...
  // Method to reconnect a specific device to its broker with connection verification
  async reconnectDevice(deviceId: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      console.log(`Attempting to reconnect device ${deviceId} to MQTT broker...`);

      // Disconnect existing client if it exists
      const existingClient = this.clients.get(deviceId);
      if (existingClient) {
        console.log(`Disconnecting existing MQTT client for device ${deviceId}`);
        existingClient.end(true); // Force close
        this.clients.delete(deviceId);
        
        // Remove topic from subscribed topics
        if (device.mqttTopic) {
          this.subscribedTopics.delete(device.mqttTopic);
        }
      }

      // Update device status to 'waiting' while reconnecting
      const waitingDevice = await storage.updateDevice(device.id, {
        status: 'waiting',
        lastSeen: new Date(),
      });
      
      if (webSocketBroadcaster && waitingDevice) {
        webSocketBroadcaster(waitingDevice);
      }

      // Wait a moment before reconnecting
      await new Promise(resolve => setTimeout(resolve, 500));

      // Use the Promise-based connection method
      try {
        const connected = await this.connectToDeviceWithVerification(device);
        
        if (connected) {
          console.log(`Reconnection successful for device ${deviceId}`);
          return { success: true, message: 'Reconnection successful' };
        } else {
          throw new Error('Connection verification failed');
        }
      } catch (connectError) {
        console.error(`Connection failed for device ${deviceId}:`, connectError);
        
        // Update device status to offline on failure
        const offlineDevice = await storage.updateDevice(device.id, {
          status: 'offline',
          lastSeen: new Date(),
        });
        
        if (webSocketBroadcaster && offlineDevice) {
          webSocketBroadcaster(offlineDevice);
        }
        
        return { success: false, message: connectError instanceof Error ? connectError.message : 'Connection failed' };
      }
    } catch (error) {
  ...
  // New method that returns a Promise resolving when MQTT connection is confirmed
  private async connectToDeviceWithVerification(device: any): Promise<boolean> {
    return new Promise((resolve, reject) => {
      try {
        ...
        const options = {
          ...
          password: device.password || '',
          reconnectPeriod: 5000,
          protocolVersion: (protocol === 'WS' || protocol === 'WSS') ? 4 : 3 as 3 | 4,
          keepalive: 60,
          rejectUnauthorized: false,
          ...((protocol === 'WS' || protocol === 'WSS') && {
            protocolId: 'MQTT' as const
        ...
        client.on('connect', () => {
          ...
          // Subscribe to device topic
          client.subscribe(device.mqttTopic, { qos: 1 }, async (error: Error | null) => {
            if (error) {
              client.end(true);
              this.clients.delete(device.deviceId);
