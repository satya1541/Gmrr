...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/devices/:deviceId/reconnect", async (req, res) => {
    try {
      const { deviceId } = req.params;
      ...
      const result = await mqttClient.reconnectDevice(deviceId);
      ...
      res.status(500).json({ message: "Failed to reconnect device to broker" });
    }
  });
  ...
  app.get("/api/history/devices/:deviceId/export", async (req, res) => {
    try {
      ...
      // Create workbook with ExcelJS for professional styling
      const workbook = new ExcelJS.Workbook();
      workbook.creator = 'ToxiShield-X';
      workbook.created = new Date();
      
      // Create Device Data worksheet
      const worksheet = workbook.addWorksheet('Device Data');
      
      // Define brand colors
      const brandBlue = { argb: 'FF1E3A5F' };
      const headerBlue = { argb: 'FF2980B9' };
      const lightGray = { argb: 'FFF5F5F5' };
      const white = { argb: 'FFFFFFFF' };
      const greenColor = { argb: 'FF27AE60' };
      const yellowColor = { argb: 'FFF39C12' };
      const redColor = { argb: 'FFE74C3C' };
      
      // Title section
      worksheet.mergeCells('A1:H1');
      const titleCell = worksheet.getCell('A1');
      titleCell.value = 'ToxiShield-X - Device History Report';
      titleCell.font = { size: 18, bold: true, color: { argb: 'FF1E3A5F' } };
      titleCell.alignment = { horizontal: 'center', vertical: 'middle' };
      worksheet.getRow(1).height = 30;
      
      worksheet.mergeCells('A3:H3');
      worksheet.getCell('A3').value = `Device: ${device.name} (${device.deviceId})`;
      worksheet.getCell('A3').font = { size: 11, color: { argb: 'FF666666' } };
      
      worksheet.mergeCells('A4:H4');
      worksheet.getCell('A4').value = `Generated: ${new Date().toLocaleString()}`;
      worksheet.getCell('A4').font = { size: 11, color: { argb: 'FF666666' } };
      
      worksheet.mergeCells('A5:H5');
      worksheet.getCell('A5').value = `Total Records: ${data.length}`;
      worksheet.getCell('A5').font = { size: 11, color: { argb: 'FF666666' } };
      
      // Header row
      const headerRow = worksheet.getRow(7);
      const headers = ['S.No', 'Date', 'Time', 'Device ID', 'Device Name', 'Alcohol Level', 'Alert Status', 'Last Offline'];
      ...
      // Data rows with alternating colors and status coloring
      data.forEach((item: DeviceData, index: number) => {
        ...
        const rowData = [
          ...
          item.alcoholLevel,
          item.alertStatus,
          latestOffline ? latestOffline.toISOString().replace('T', ' ').split('.')[0] : '-'
        ];
        ...
        rowData.forEach((value, i) => {
          ...
          if (index % 2 === 1) {
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: lightGray };
          }
          
          // Color-code Alert Status column
          if (i === 6) {
            if (item.alertStatus === 'Normal') {
              cell.font = { bold: true, color: greenColor };
            } else if (item.alertStatus === 'Moderate Drunk') {
              cell.font = { bold: true, color: yellowColor };
            } else if (item.alertStatus === 'Completely Drunk') {
              cell.font = { bold: true, color: redColor };
            }
          }
        });
      });
      ...
      worksheet.columns = [
        ...
        { key: 'alcoholLevel', width: 16 },
        { key: 'alertStatus', width: 20 },
        { key: 'lastOffline', width: 24 }
      ];
      
      // Create Offline Events worksheet
      const offlineSheet = workbook.addWorksheet('Offline Events');
      
      offlineSheet.mergeCells('A1:D1');
      const offlineTitleCell = offlineSheet.getCell('A1');
      offlineTitleCell.value = 'ToxiShield-X - Offline Events Report';
      offlineTitleCell.font = { size: 18, bold: true, color: { argb: 'FF1E3A5F' } };
      offlineTitleCell.alignment = { horizontal: 'center', vertical: 'middle' };
      offlineSheet.getRow(1).height = 30;
      
      offlineSheet.mergeCells('A3:D3');
      offlineSheet.getCell('A3').value = `Device: ${device.name} (${device.deviceId})`;
      offlineSheet.getCell('A3').font = { size: 11, color: { argb: 'FF666666' } };
      
      // Offline header row
      const offlineHeaderRow = offlineSheet.getRow(5);
      const offlineHeaders = ['S.No', 'Device ID', 'Device Name', 'Went Offline At'];
      offlineHeaders.forEach((header, i) => {
        const cell = offlineHeaderRow.getCell(i + 1);
        cell.value = header;
        cell.font = { bold: true, color: white };
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: headerBlue };
        cell.alignment = { horizontal: 'center', vertical: 'middle' };
      ...
      offlineHeaderRow.height = 22;
      
      // Offline data rows
      const offlineData = sortedOfflineEvents.length
        ? sortedOfflineEvents.map((event, index) => [
            index + 1,
            device.deviceId,
            device.name,
            event.offlineAt.toISOString().replace('T', ' ').split('.')[0]
          ])
        : [[1, device.deviceId, device.name, 'No offline events in selected range']];
      
      offlineData.forEach((rowData, index) => {
        const rowNum = index + 6;
        const row = offlineSheet.getRow(rowNum);
        rowData.forEach((value, i) => {
          const cell = row.getCell(i + 1);
          cell.value = value;
