...
export function DeviceCard({ device, latestData, onRefresh }: DeviceCardProps) {
  ...
  // Mutation for reconnecting device to MQTT broker
  const reconnectMutation = useMutation({
    mutationFn: async (deviceId: string) => {
      await apiRequest('POST', `/api/devices/${deviceId}/reconnect`);
    },
    onSuccess: () => {
      // Invalidate devices query to refresh UI state
  ...
  const handleRefresh = async () => {
    ...
    try {
      // Call the reconnect API to retry connection to the MQTT broker
      await reconnectMutation.mutateAsync(device.deviceId);
